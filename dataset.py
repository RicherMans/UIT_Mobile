...
class WeakRandomCropHDF5Dataset(WeakHDF5Dataset):
    def __init__(self, data_frame, chunk_length:float, num_classes:int, sample_rate:int =16000, smooth:bool=False):
        super(WeakRandomCropHDF5Dataset, self).__init__(data_frame, num_classes=num_classes)
...
class UnlabeledRandomChunkedHDF5Dataset(torch.utils.data.Dataset):
    def __init__(self,data_df, chunk_length:int = 2, sample_rate:int = 16000, num_classes=527,):
        super(UnlabeledRandomChunkedHDF5Dataset, self).__init__()
        self._dataframe =data_df
        self.sample_rate = sample_rate
        self.chunk_length = chunk_length
        self.chunk_length = int(self.sample_rate * self.chunk_length)
        self.num_classes = num_classes
        self._datasetcache = {}

        logger.info(f"Using random chunks of length {self.chunk_length}")
...
class BalancedSampler(torch.utils.data.WeightedRandomSampler):
    def __init__(self, labels_df: pd.Series, offset:int = 100, random_state= None):
        ...
        occurances = single_labels_df.groupby('label')['index'].apply(
            len)
        occurances = occurances.sort_index()
        weights = 1000. / (occurances + offset
                            )  # Offset classes with low prob
        weights = weights.to_dict()
        sample_weights = labels_df.apply(lambda x: sum(
            [weights[class_id] for class_id in x])).values
        super().__init__(sample_weights, num_samples=len(sample_weights))
